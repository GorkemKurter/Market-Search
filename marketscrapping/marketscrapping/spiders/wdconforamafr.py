...
class WdconforamafrSpider(scrapy.Spider):
    ...
    start_urls = ["file:///C:/Users/gorkemk/PycharmProjects/Market-Search/marketscrapping/HTML%20Files/wdconforamafr/page1.html"]
    page_counter = 0
    counter = 0

    def parse(self, response):

        product_urls = response.css("#addToCartForm > div.column-product.column-product_middle > div.awk-detail-product > div.typo-paragraphe.c-r_product_name > a::attr(href)").getall()
        for i in range(len(product_urls)):
            try:
                if response.css(f"#contentSegment > div.c-r_descent-container > div > article:nth-child({i}) div.awk-desc-product.product-pointfort li:nth-child(1) span::text").get() == None:
                    ...
                    template_string = response.css(f"#contentSegment > div.c-r_descent-container > div > article:nth-child(7) a.extendLink div.c-r_refProductName-grp span::text").get()
        ...
        self.page_counter += 1
        next_url = fr"file:///C:/Users/gorkemk/PycharmProjects/Market-Search/marketscrapping/HTML%20Files/wdconforamafr/page{self.page_counter}.html"
